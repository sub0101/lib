...
import { AddRequest } from "../../react-queries/api/requestTracking"
...
const requestConfirmation = (data)=>{
  const confirmReturn = window.confirm(`Are you sure you want to return "${data.bookTitle}"?`)

  if (confirmReturn) {
    payload.bookId= data.id 
    payload.requestType = "issue"
        ...
        {filteredBooks &&  filteredBooks.map((book) => (
          <div className={styles.bookCard} key={book.id}>
            ...
            <div className={styles.bookDetails}>
              ...
              {userInfo.Role == "owner" && <button className={styles.editButton}>Edit</button> }
               {userInfo.Role == "reader" &&  <button className={styles.editButton}   onClick={()=> requestConfirmation({id:book.id  ,bookTitle :book.title})} > Request</button> }
               {userInfo.Role == "owner" &&   <button className={styles.deleteButton}>Delete</button> }
              </div>
            </div>
          </div>
        ))}
