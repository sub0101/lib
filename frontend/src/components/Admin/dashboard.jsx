...
function Dashboard() {
 ...
 useEffect(() => {
  if (isSuccess) {
    const totalBooks = booksQuery.map((book) => book.totalCopies).reduce((sum, count) => sum + count, 0);
    const availableCopies = booksQuery.map((book) => book.availableCopies).reduce((sum, count) => sum + count, 0);
